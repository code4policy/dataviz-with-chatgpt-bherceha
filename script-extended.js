// Load CSV, compute all reasons, and render a horizontal D3 bar chart
...
function render(data) {
  ...
  document.getElementById('sub-headline').textContent = 'All reasons for Boston 311 calls in 2025.';
  ...
  const height = all.length * 42; // band size * count
  ...
  const x = d3.scaleLinear()
    .domain([0, d3.max(all, d=>d.Count)]).nice()
    .range([0, width]);

  const y = d3.scaleBand()
    .domain(all.map(d=>d.reason))
    .range([0, height])
    .padding(0.15);
  ...
  // bars
  const bars = g.selectAll('.bar-group').data(all).join('g').attr('class','bar-group').attr('transform', d=>`translate(0,${y(d.reason)})`);
